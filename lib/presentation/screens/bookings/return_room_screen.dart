...
class _ReturnRoomScreenState extends State<ReturnRoomScreen> {
  ...
  void _submit() {
    ...
    final myRoomState = context.read<MyRoomBloc>().state;
    final activeBooking = myRoomState is MyRoomLoaded
        ? myRoomState.activeBooking
        : null;
    if (activeBooking == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Không tìm thấy hợp đồng đang hoạt động.'),
        ),
      );
      context.go('/');
      return;
    ...
    context.read<BookingBloc>().add(
      RequestReturnBookingEvent(reason: _reasonController.text.trim()),
    );
  }
    ...
    return WillPopScope(
      ...
      child: BlocConsumer<BookingBloc, BookingState>(
        ...
        builder: (context, state) {
          ...
          final hasRoom = activeBooking != null && room != null;
          ...
          return Scaffold(
            ...
            body: Padding(
              ...
              child: AbsorbPointer(
                absorbing: isLoading,
                child: hasRoom
                    ? _buildForm(context, room!, isLoading)
                    : Center(
                        child: Padding(
                          padding: const EdgeInsets.all(AppSizes.paddingMedium),
                          child: Text(
  ...
  Widget _buildForm(BuildContext context, dynamic room, bool isLoading) {
    return Form(
      key: _formKey,
      child: ListView(
        children: [
